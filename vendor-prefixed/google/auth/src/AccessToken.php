...
use Mihdan\ReCrawler\Dependencies\phpseclib\Crypt\RSA;
use Mihdan\ReCrawler\Dependencies\phpseclib\Math\BigInteger;
...
class AccessToken
{
    ...
    public function verify($token, array $options = [])
    {
        ...
        try {
            ...
            // firebase/php-jwt 3+
        } catch (\Mihdan\ReCrawler\Dependencies\ExpiredException $e) {
            // firebase/php-jwt 2
        } catch (SignatureInvalidException $e) {
            // firebase/php-jwt 3+
        } catch (\Mihdan\ReCrawler\Dependencies\SignatureInvalidException $e) {
            // firebase/php-jwt 2
    ...
    {
        // @codeCoverageIgnoreStart
        if (!\class_exists('Mihdan\\ReCrawler\\Dependencies\\SimpleJWT\\JWT')) {
    ...
    private function setPhpsecConstants()
    {
        if (\filter_var(\getenv('GAE_VM'), \FILTER_VALIDATE_BOOLEAN)) {
            ...
            if (!\defined('Mihdan\\ReCrawler\\Dependencies\\CRYPT_RSA_MODE')) {
                \define('Mihdan\\ReCrawler\\Dependencies\\CRYPT_RSA_MODE', RSA::MODE_OPENSSL);
            }
        }
    }
    /**
    ...
    {
        $class = \class_exists('Mihdan\\ReCrawler\\Dependencies\\Firebase\\JWT\\JWT') ? 'Firebase\\JWT\\JWT' : 'JWT';
