...
class OAuth2 implements FetchAuthTokenInterface
{
    ...
    private function jwtDecode($idToken, $publicKey, $allowedAlgs)
    {
        if (\class_exists('Mihdan\\ReCrawler\\Dependencies\\Firebase\\JWT\\JWT')) {
            return \Mihdan\ReCrawler\Dependencies\Firebase\JWT\JWT::decode($idToken, $publicKey, $allowedAlgs);
        }
        return \Mihdan\ReCrawler\Dependencies\JWT::decode($idToken, $publicKey, $allowedAlgs);
    }
    private function jwtEncode($assertion, $signingKey, $signingAlgorithm, $signingKeyId = null)
    {
        if (\class_exists('Mihdan\\ReCrawler\\Dependencies\\Firebase\\JWT\\JWT')) {
            return \Mihdan\ReCrawler\Dependencies\Firebase\JWT\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
        }
        return \Mihdan\ReCrawler\Dependencies\JWT::encode($assertion, $signingKey, $signingAlgorithm, $signingKeyId);
    }
    /**
